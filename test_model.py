...
model = AutoModel.from_pretrained(
    path, torch_dtype=torch.bfloat16, load_in_8bit=True, low_cpu_mem_usage=True, use_flash_attn=True, trust_remote_code=True, device_map=device_map
).eval()
tokenizer = AutoTokenizer.from_pretrained(path, trust_remote_code=True, use_fast=False)

# set the max number of tiles in `max_num`
pixel_values = load_image("media/image1.jpg", max_num=12).to(torch.bfloat16).cuda()
...
pixel_values1 = load_image("media/image1.jpg", max_num=12).to(torch.bfloat16).cuda()
pixel_values2 = load_image("media/image2.jpg", max_num=12).to(torch.bfloat16).cuda()
