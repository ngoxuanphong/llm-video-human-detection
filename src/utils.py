...
from src import MAX_FRAMES, SAVE_FORMAT, TEMP_DIR, console, logger
...
def save_analysis_frames_to_temp(frames, folder_dir=TEMP_DIR):
    ...
    try:
        ...
        save_dir = os.path.join(folder_dir, f"{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        ...
        info_path = os.path.join(save_dir, "analysis_info.txt")
        with open(info_path, "w", encoding="utf-8") as f:
            f.write("Fall Detection Analysis \n")
            f.write(f"{'='*40}\n")
        ...
        if SAVE_FORMAT == "images":
            logger.info(f"[green]{icon}[/green] Đã lưu {len(frame_arrays)} khung hình tại: [cyan]{save_dir}[/cyan]", extra={"markup": True})
        else:
            logger.info(f"[green]{icon}[/green] Đã lưu {len(saved_files)} file ({SAVE_FORMAT}) tại: [cyan]{save_dir}[/cyan]", extra={"markup": True})
